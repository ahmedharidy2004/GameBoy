...
GRAVITY EQU 5 ; When Falling Decrement Gravity
PLAYER_SPEED EQU 5 ; Player Speed
JUMP_STRENGTH EQU 80 ; When Jump Increment Strength
...
BORDERS EQU 9 ; Borders from All Direction
STATE_PLAYERS DCD 0 ; (0xXXXX XXXX) P1, P2 (0 Falling)
; Score
SCORE DCD 0
COLOR DCD 0
SCOREX DCD 0
SCOREY DCD 0
BG DCD 0
HEART DCD 0 ; Increments Heal
COIN DCD 0 ; Increments Score
;COLOURS
BLACK EQU 0X0000
BLUE EQU 0x001F
WHITE EQU 0XFFFF
...
FIREDORTIREDLOOP
	...
	BL MOVE_PLAYER2
	BL MOVE_BULLETS
	BL CHECK_BULLETS_END
	BL CHECK_BULLET_COLLISION
	BL DrawBorders
	BL DrawRods
	BL DrawPlayers
...
CHECKLOOP2
	...
	CMP R2, #2 ; Right
	BNE SKIPRIGHTCHECKCOLLISION2
	;CMP THE Y IF NOT EQUAL THEN LOOP ON ANOTHER ROD
	CMP R9, R12 
	SUBGE R4, R9, R12
	SUBLT R4, R12, R9
	CMP R4, #PLAYER_HEIGHT - 3
	BGT LOOPANOTHER2
	...
	MOV R4, #0xFFFF
	CMP R7, R12	
	BGT SKIPRIGHTCHECKCOLLISION2
	SUB R4, R12, R7
	CMP R4, #PLAYER_WIDTH
	BGT LOOPANOTHER2
	MOV R10, #1
	B CHECKFINISHED2
SKIPRIGHTCHECKCOLLISION2
	;CMP THE Y IF NOT EQUAL THEN LOOP ON ANOTHER ROD
	CMP R9, R12 
	SUBGE R4, R9, R12
	SUBLT R4, R12, R9
...
CHECKMOVE1
    ...
    CMP R5, #1
	BNE SKIPUPCHECKCOLLISION1
	CMP R3, #PLAYER_HEIGHT
	BLE SKIPUPCHECKCOLLISION1
	LDR R6, =STATE_PLAYERS
	LDR R6, [R6]
	AND R6, R6, #0xFF
	CMP R6, #1
	BNE SKIPUPCHECKCOLLISION1
	SUB R6, R2, #PLAYER_WIDTH
	ADD R8, R2, #PLAYER_WIDTH
	ADD R7, R3, #PLAYER_HEIGHT
	SUB R7, R7, #JUMP_STRENGTH
	ADD R9, R7, #JUMP_STRENGTH
	MOV R10, #CYAN
	BL DrawRect
	SUB R3, #JUMP_STRENGTH
SKIPUPCHECKCOLLISION1
    CMP R5, #2
	SUBEQ R6, R2, #PLAYER_WIDTH
	ADDEQ R8, R6, #PLAYER_SPEED
	SUBEQ R7, R3, #PLAYER_HEIGHT
	ADDEQ R9, R3, #PLAYER_HEIGHT
	MOVEQ R10, #CYAN
	BLEQ DrawRect
    ADDEQ R2, #PLAYER_SPEED
    
    CMP R5, #3
	ADDEQ R6, R2, #PLAYER_WIDTH
	SUBEQ R6, R6, #PLAYER_SPEED
	ADDEQ R8, R6, #PLAYER_SPEED
	SUBEQ R7, R3, #PLAYER_HEIGHT
	ADDEQ R9, R3, #PLAYER_HEIGHT
	MOVEQ R10, #CYAN
...
CHECKMOVE2
    ...
    CMP R5, #1
	BNE SKIPUPCHECKCOLLISION2
	CMP R3, #PLAYER_HEIGHT
	BLE SKIPUPCHECKCOLLISION2
	LDR R6, =STATE_PLAYERS
	LDR R6, [R6]
	LSR R6, R6, #16
	CMP R6, #1
...
LOOP_COLLISIONBULLETS1
	...
	BGT GO_NEXTCOLLISIONLOOP1
	BL HIT_PLAYER1 ; R5 HAS THE BULLET OFFSET IT WILL BE PASSES TO THE FUNCTION AND IT SHOULD ERASE IT
...
LOOP_COLLISIONBULLETS2
	...
	CMP R3, R1
	BGT GO_NEXTCOLLISIONLOOP2
	CMP R4, R8 ; COMPARE WITH THE UPPER PART THE BULLET Y MYST BE LOWER OR EQUAL IT
	BLT GO_NEXTCOLLISIONLOOP2
	CMP R4, R7 ; COMPARE WITH THE LOWER PART THE BULLET Y MUST BE GREATER OR EQUAL IT
	BGT GO_NEXTCOLLISIONLOOP2
	BL HIT_PLAYER2 ; R2 HAS THE BULLET OFFSET IT WILL BE PASSES TO THE FUNCTION AND IT SHOULD ERASE IT
